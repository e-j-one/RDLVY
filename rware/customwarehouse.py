...
class Agent(Entity):
    ...
    def __init__(self, x: int, y: int, dir_: Direction, line_dir_: Line_Direction, msg_bits: int):
        Agent.counter += 1
        super().__init__(Agent.counter, x, y)
        self.dir = dir_
        self.line_dir = line_dir_ # added component
        self.message = np.zeros(msg_bits)
        self.req_action: Optional[Action] = None
        self.carrying_shelf: Optional[Shelf] = None
        self.canceled_action = None
        self.prev_action: Action = None # added component
    ...
    def req_location(self, grid_size) -> Tuple[int, int]:
        if self.req_action != Action.FORWARD:
            if self.prev_action == self.req_action:
                if self.line_dir == Line_Direction.LEFT:
                    self.line_dir = Line_Direction.RIGHT
                else:
                    self.line_dir = Line_Direction.LEFT
            self.prev_action = self.req_action
...
class CustomWarehouse(gym.Env):
    ...
    def __init__(
        ...
        self.pickup_places: List[Tuple[int, int]] = []
    ...
    def _make_layout_from_str(self, layout):
        ...
        self.grid = np.zeros((_COLLISION_LAYERS, *self.grid_size), dtype=np.int32)
        self.highways = np.zeros(self.grid_size, dtype=np.int32)
        self.highways_status = np.zeros(self.grid_size + (2,), dtype=Line_Status) 
        ...
        for y, line in enumerate(lines):
            for x, char in enumerate(line):
                ...
                if char.lower() == "g":
                    ...
                    self.highways[y, x] = 1
                    self.highways_status[y, x, :] = Line_Status.EMPTY 
                elif char.lower() == ".":
                    self.highways[y, x] = 1
                    self.highways_status[y, x, :] = Line_Status.EMPTY 
                # elif char == "p":
                    # self.pickup_places.append((x, y))
                    # self.highways[y, x] = 1
                    # self.highways_status[y, x, :] = Line_Status.EMPTY 
                                    ...
                                    OrderedDict(
                                        {
                                            ...
                                            "line_direction": spaces.Discrete(2),
                                        }
                                    )
                                ),
    ...
    def _get_highway_status(self, x: int, y: int) -> np.array([Line_Status ,Line_Status]):
        return self.highways_status[y, x, :]

    def _is_near_highway(self, x: int, y: int) -> bool:
        if not self.highways[y, x]:
            for r1 in [-1, 1]:
                if x + r1 > self.grid_size[1] - 1 or x + r1 < 0: 
                    r1 = 0
                if self.highways[y, x+r1]:
                    return 1
            for r2 in [-1, 1]:
                if y + r2 > self.grid_size[0] - 1 or y + r2 < 0:
                    r2 = 0
                if self.highways[y+r2, x]:
                    return 1 
        return 0
    ...
    def _make_obs(self, agent):
        ...
        if self.fast_obs:
            ...
            line_direction[agent.line_dir.value] = 1.0
            obs.write(line_direction)
            ...
            for i, (id_agent, id_shelf) in enumerate(zip(agents, shelfs)):
                if id_agent == 0:
                    ...
                    obs.write(direction)
                    line_direction = np.zeros(2)
                    line_direction[self.agents[id_agent - 1].line_dir.value] = 1.0
                    obs.write(line_direction)
                    if self.msg_bits > 0:
        ...
        obs["self"] = {
            ...
            "line_direction": agent.line_dir.value
        }
        # --- sensor data
    ...
    def reset(self):
        ...
        self.shelfs = [
            ...
            if not self._is_highway(x, y)# and self._is_near_highway(x, y)
        ]
        
        # spawn agents at random locations
        # Caution! the agents should be near to the highway or on the highway
        # Caution! the target shelfs should be near to the highway
        target_shelfs_idx = []
        possible_locs = []
        for y, x in zip(
            np.indices(self.grid_size)[0].reshape(-1),
        ...
        target_shelfs_idx = np.array(target_shelfs_idx)
        possible_locs = np.array(possible_locs)
        agent_locs_idx = np.random.choice(possible_locs.shape[0], size=self.n_agents, replace=False)
        agent_locs = possible_locs[agent_locs_idx]
        agent_locs = (agent_locs[:, 1], agent_locs[:, 0])
        ...
        agent_line_dirs = np.random.choice([ld for ld in Line_Direction], size=self.n_agents)
        self.agents = [
            Agent(x, y, dir_, line_dir_, self.msg_bits)
            for y, x, dir_, line_dir_ in zip(*agent_locs, agent_dirs, agent_line_dirs)
        ]
        
        self._recalc_grid()
        self.shelfs = np.array(self.shelfs)
        self.request_queue = list(
            np.random.choice(self.shelfs[np.where(target_shelfs_idx==np.ones(target_shelfs_idx.shape[0]))[0]],
                            size=self.request_queue_size, replace=False)
        )
        # self.pickup_places = np.array(self.pickup_places)
        # self.request_queue_idx = np.random.choice(len(self.pickup_places), size=self.request_queue_size)
        # self.request_queue = [Shelf(x, y) for (x, y) in self.pickup_places[self.request_queue_idx]]
        
        # self.shelfs = list(self.shelfs)
...
if __name__ == "__main__":
    env = CustomWarehouse(9, 8, 3, 10, 3, 1, 5, None, None, RewardType.GLOBAL)
    env.reset()
    import time
